...
function FeatureCard({ icon, title, desc }) {
  return (
    ...
    >
      ...
      <div style={{ color: "#8a8ab0", fontSize: "0.85rem", lineHeight: 1.6 }}>
        {desc}
      </div>
    </div>
  );
...
export default function Home() {
  ...
  useEffect(() => {
    const loadImages = async () => {
      ...
      for (let i = 0; i <= 69; i++) {
        ...
        img.src = `/Animations/animation1/lego kid animation _${num}.jpg`;
      ...
      // Draw first frame
      if (scrubCanvasRef.current && loadedImages[0]) {
        const ctx = scrubCanvasRef.current.getContext("2d");
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (!scrubContainerRef.current || !imagesLoaded) return;

      const rect = scrubContainerRef.current.getBoundingClientRect();
      const scrollStart = 0;
      const scrollDistance = rect.height - window.innerHeight;

      let progress = (window.scrollY - rect.top) / scrollDistance;
      progress = Math.min(Math.max(progress, 0), 1);
      setScrollProgress(progress);

      if (scrubCanvasRef.current && imagesRef.current.length > 0) {
    ...
    try {
      ...
      toast.error(
        e.response?.data?.error ||
          "Detection failed. Ensure the AI server is running.",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <div
        ...
        style={{ position: "relative", height: "130vh", width: "100%" }}
      >
        <div
          style={{
            position: "sticky",
        ...
        >
          <canvas
            ...
            style={{
              ...
              display: "block",
              opacity:
                scrollProgress > 0.95 ? 1 - (scrollProgress - 0.95) * 20 : 1, // fade out right at the end to merge smoothly
            }}
          />

          {/* Background backdrop blur behind text to improve readability of intricate backgrounds */}
          <div
            style={{
              position: "absolute",
              top: 0,
              ...
              width: "100%",
              height: "100%",
              background: "rgba(10, 10, 22, 0.05)", // extremely slight tint
          ...
          {/* Subtle gradient so edges blend, but center remains clear for knockout effect */}
          <div
            style={{
              position: "absolute",
              top: 0,
              ...
              height: "100%",
              background:
                "linear-gradient(to right, rgba(10,10,22,0.4) 0%, rgba(10,10,22,0) 20%, rgba(10,10,22,0) 80%, rgba(10,10,22,0.4) 100%)",
              pointerEvents: "none",
            }}
          />
          <div
            style={{
              ...
              height: "15%",
              background:
                "linear-gradient(to top, rgba(10,10,22,1) 0%, transparent 100%)",
              pointerEvents: "none",
            }}
          />

          {/* Fading Texts with Knockout/Exclusion Effect */}
          <div
            style={{
              position: "absolute",
              top: "50%",
              ...
              alignItems: "center",
              justifyContent: "center",
              mixBlendMode: "exclusion", // This creates the knockout text effect
              color: "#ffffff",
              pointerEvents: "none",
            }}
          >
            <div
              style={{
                opacity: Math.max(0, 1 - scrollProgress * 3), // fades out from 0 to 0.33
            ...
            >
              ...
              >
                Watch loose parts come together down the scroll.
              </p>
            </div>
              ...
              style={{
                opacity:
                  scrollProgress > 0.3 && scrollProgress < 0.7
                    ...
                    : scrollProgress >= 0.7
                      ? Math.max(0, 1 - (scrollProgress - 0.7) * 5)
                      : 0,
            ...
            >
              ...
              >
                YOLOv8 instantly recognizes every precise part.
              </p>
            </div>
            ...
            <div
              style={{
                opacity:
                  scrollProgress > 0.7
                    ? Math.min(1, (scrollProgress - 0.7) * 5)
                    : 0,
                transform: `translateY(${(0.9 - scrollProgress) * -50}px)`,
                transition: "opacity 0.1s",
                position: "absolute",
                width: "100%",
              }}
            >
              ...
              >
                Upload your images below to start the magic.
              </p>
            </div>
          </div>
        </div>
